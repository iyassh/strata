...
| T4 | *(paths part resolved 2026-09-23: converters default to data/raw/, env-var override — the CLI part remains)* **No fault-free-only entry point**: shipped `processheal run` CLI is the stale v1 single-channel pipeline (also demands a --faulty file and equipment.ttl); processheal-web crashes at import on a fresh clone | BLOCKING |
