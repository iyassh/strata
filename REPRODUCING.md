...
| X29 refrigerant-side observability (pre-registered): LBNL simulated RTU (24 faults, refrigerant pressures/temperatures recorded): condenser fouling 5/5 and evaporator fouling 5/5 monotone, line restrictions 8/8, charge faults 2/6; 20/24 at 1/29 holdout days; conformance on none; no falsifier | `week0_audit_rtu_sim.json`, `benchmark_v6_rtu_sim.json`, `union_fpr_rtu_sim.json`, `x29_refrigerant_observability.json` (`configs/lbnl_rtu_sim`, `scripts/08_convert_rtu_sim.py`, `x29_refrigerant_observability.py`; `docs/plans/2026-09-24-x29-refrigerant-observability-prereg.md`; guard `tests/test_x29_refrigerant_observability.py`) |
