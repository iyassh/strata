...
| X25 statistical repair (pre-registered): calibration slice (`calibration_days_per_month: 8`) makes the conformance holdout rows out-of-sample; uniform rule-of-three floor; counts unchanged on all five systems, no status flips; deployed budgets PFPU 5→7, SFPU 4→6 of 96 (model rows 1→4, 1→5); byte-identical regression before the configs changed | all `benchmark_v6_*.json`, `union_fpr_*.json`, `crywolf.json`, `x25_statistical_repair.json` (`scripts/x25_statistical_repair.py`; `docs/plans/2026-09-24-x25-statistical-repair-prereg.md`; guard `tests/test_x25_statistical_repair.py`) |
