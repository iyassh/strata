...
for s in sdahu fcu ddahu; do uv run python scripts/x31_field_conditions.py --system $s; done   # X31 -> outputs/x31_field_conditions_<s>.json (~1 h)
uv run python scripts/x31_ledger.py                           # -> outputs/x31_field_conditions.json
uv run python scripts/x33_coverage.py --system sdahu --before 8f06f19   # X33a -> outputs/x33_coverage_sdahu.json (after benchmark.py/union_fpr.py sdahu)
uv run python scripts/x33_coverage.py --system pfpu --before 6b7512a    # X33b -> outputs/x33_coverage_pfpu.json
uv run python scripts/x33_residual_credits.py --system pfpu            # -> outputs/x33_residual_credits_pfpu.json (which rule carries which scenario)
uv run python scripts/x33_coverage.py --system sfpu --before 028a941    # X33c -> outputs/x33_coverage_sfpu.json
uv run python scripts/x33_residual_credits.py --system sfpu            # -> outputs/x33_residual_credits_sfpu.json
uv run python scripts/x33_coverage.py --system ddahu --before c40144d   # X33d -> outputs/x33_coverage_ddahu.json
uv run python scripts/x33_residual_credits.py --system ddahu           # -> outputs/x33_residual_credits_ddahu.json (~12 min)
uv run python scripts/x33_coverage.py --system fcu --before d8f5064     # X33e -> outputs/x33_coverage_fcu.json
uv run python scripts/x33_residual_credits.py --system fcu             # -> outputs/x33_residual_credits_fcu.json
uv run python scripts/component_attribution.py                         # X34 -> outputs/component_attribution.json (~10 min)
...
| X31 field conditions (pre-registered + Amendment 1): sensor error/quantisation, change-of-value logging with gaps, schedule shifts and all three injected into SDAHU/DDAHU/FCU, re-fitted and scored through the library facade against a clean arm of the same facade: budget holds on every condition (worst 3/96); noise and COV cost ≤ 3 detections, combined ≤ 1; the schedule shift gains 3 DDAHU sensor-bias detections (P3 failed upward); the absence channel not exercised; the first pass exposed and repaired a facade drift (pooled residual null) | `x31_field_conditions.json`, `x31_field_conditions_{sdahu,ddahu,fcu}.json` (`scripts/x31_field_conditions.py`, `x31_ledger.py`; `docs/plans/2026-09-24-x31-field-conditions-prereg.md` + Amendment 1; guards `tests/test_x31_field_conditions.py`, `tests/test_facade_residual_null.py`) |
| X33a coverage audit, SDAHU (pre-registered series): all 30 columns mapped or excluded with a reason; fan-side residuals and zone-temperature oscillation added; 14/14 and 1/96 unchanged, new residuals 0/79 false alarms each, model rows byte-identical, residual channel newly credited on two stuck-coil scenarios | `x33_coverage_sdahu.json`, `benchmark_v6_sdahu.json`, `union_fpr_sdahu.json` (`configs/lbnl_sdahu`, `scripts/x33_coverage.py`; `docs/plans/2026-09-25-x33-coverage-series-prereg.md`; guards `tests/test_x33_coverage.py`, `tests/test_sensor_coverage.py`) |
| X33b coverage audit, PFPU: all 109 columns mapped; 22 → 26/30 (airside reheat fouling moderate and severe via zone discharge-minus-primary airflow; room-temperature bias +2/+4 °C via return-minus-zone residuals); no loss; 7 → 8/96; every new rule ≤ 1 holdout day; model rows identical | `x33_coverage_pfpu.json`, `x33_residual_credits_pfpu.json`, `benchmark_v6_pfpu.json`, `union_fpr_pfpu.json` (`configs/lbnl_pfpu`) |
| X33c coverage audit, SFPU: all 109 columns mapped; 21 → 25/29 (fan restriction, airside reheat fouling moderate and severe via zone-fan specific power; +2 °C bias); no loss; 6 → 9/96; model rows identical | `x33_coverage_sfpu.json`, `x33_residual_credits_sfpu.json`, `benchmark_v6_sfpu.json`, `union_fpr_sfpu.json` (`configs/lbnl_sfpu`) |
| X33d coverage audit, DDAHU: all 114 columns mapped; 45 → 50/55 (hot-deck static bias −2/−4 and temperature bias +2 °C via mixing-box residuals; heating waterside fouling minor via pump bypass; cooling airside fouling moderate via fan specific power); no loss; 3 → 3/96; all 25 new rules 0/75; model rows identical | `x33_coverage_ddahu.json`, `x33_residual_credits_ddahu.json`, `benchmark_v6_ddahu.json`, `union_fpr_ddahu.json` (`configs/lbnl_ddahu`) |
| X33e coverage audit, FCU: readers for two unread columns; 40 → 43/47 (damper leaks 20 % and 50 % via outdoor-air flow fraction at minimum position; heating waterside fouling moderate via the fan's operating point); no loss; 4 → 5/96; model rows identical. Series: 142 → 158/175, 21 → 26/480 | `x33_coverage_fcu.json`, `x33_residual_credits_fcu.json`, `benchmark_v6_fcu.json`, `union_fpr_fcu.json` (`configs/lbnl_fcu`) |
| X34 component attribution (pre-registered): carrying rule vs seeded component over 158 detections — exact 109, same subsystem 22, wrong 20, unnamed 7; F-X34.a fired (wrong 12.7 % > 10 %): component-level claim withdrawn, subsystem-level reported | `component_attribution.json` (`scripts/component_attribution.py`; `docs/plans/2026-09-25-x34-component-attribution-prereg.md`; guard `tests/test_component_attribution.py`) |
...
| Regression gate, X33 coverage series (2026-09-25): a tenth artefact frozen — `x7_downsample.json` at 8f06f19 — because the 15-minute downsampling experiment of 2026-09-11 regenerates differently under configurations that now read every column; its robustness claim describes the configurations of its day, not the shipped ones (threat to validity recorded in PHASE12) | `scripts/regress.py` FROZEN |
| Regression gate at the end of the X33 series (2026-09-25 08:05, HEAD 455436c): 47 live artefacts identical, ten frozen, two more frozen — `x25_statistical_repair.json` (295dfe3) and `x28_coil_ua.json` (2cbf814), whose 'after' side reads the live scorecards that the coverage series moved; both are closed records of their experiments | `scripts/regress.py` FROZEN |
| Regression gate at the freeze (2026-09-24 22:53, HEAD 126af8c): 52 artefacts identical, nine frozen, two changed and repaired — the X29 ledger's Amendment 1 source check compared against HEAD instead of its regeneration commit (now pinned to 3bb4fda), and `discovery_predicts_transfer.json` carried the sealed Q file's pre-split training-day counts in its `_aux` fields (regenerated from the identical Q file; rho, p and falsifiers unchanged) | `scripts/regress.py` |
