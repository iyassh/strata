...
| L59 | X33c (coverage series, SFPU): all 109 columns mapped, zone-fan specific-power and pressure-per-flow² residuals per zone. 21 → **25 of 29**: the VAV fan restriction and both airside reheat-fouling misses (severe, moderate) through zone S fan power per cfm (261, 261, 145 days), the +2 °C bias through discharge-minus-supply and return-minus-zone residuals; no loss; deployed 6 → 9/96 (three residual days on four different rules, all kept under the adoption rule); model rows identical. | A continuously running zone fan is a permanent witness of downstream airflow resistance: 'same power, less air' is the signature of both a restricted fan and a fouled coil face | Every X32 prediction for this unit held. The one day of budget headroom left is the series' first real cost. **Rule: when a channel's holdout days approach the budget, name the rule that could be removed and what it would cost, in the write-up, before anyone tunes.** |
| L60 | X33d (coverage series, DDAHU): all 114 columns mapped (35 added); mixing-box statics and entering-air temperatures as redundant copies of the deck sensors, deck-fan and pump readers. 45 → **50 of 55**: all three sensor-bias misses (hot-deck static −2/−4 in.wg, hot-deck temperature +2 °C) at 282/282 days each through the box residuals, heating waterside fouling minor through the hot-water pump bypass (282 days), cooling airside fouling moderate through cold-deck fan specific power (20 days, first alarm day 14); no loss; deployed 3 → 3/96; all 25 new rules 0/75; model rows identical; three scenarios detected earlier. | The dataset records an unbiased second copy of every biased deck sensor; the configuration had not read it | Sensor bias is the easiest fault class once redundancy is mapped. **Rule: map every redundant measurement pair before writing any single-sensor band.** |
