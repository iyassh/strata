...
| L55 | X31 first pass: every DDAHU condition, including a schedule shift that touches only the occupancy signal, detected 50/55 against the scorecard's 45, the same four scenarios gained each time. Cause: the library facade (`pipeline.fit`) still pooled the residual null over channel-days (X25 step 3, 3/509) while `scripts/benchmark.py` had moved to the per-day null (step 4, 3/124); the regression gate regenerates through the scripts and could not see the drift. Facade repaired (X31 Amendment 1), guard `tests/test_facade_residual_null.py`, all arms re-run against a clean arm of the same facade. | The published scorecards were right; the importable library ran a looser gate than they report | **Rule: every experiment that scores through the library needs a clean arm through the same code path, and a guard pins each library statistic to the scorecard that reports it.** |
