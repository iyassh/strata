...
| L61 | X33e (coverage series, FCU): no unmapped column, but two mapped columns no rule read; readers for outdoor-air flow at the minimum damper position and fan specific power. 40 → **43 of 47**: both remaining damper leaks (20 %, 50 %) at 261/261 days through the outdoor-air flow fraction, and heating waterside fouling moderate through the fan being driven to a speed the healthy year never used (an operating-point witness); no loss; 4 → 5/96; model rows identical. **Series total: 142 → 158 of 175 scenarios, 21 → 26 false-alarm days of 480, nothing lost, process-mining rows byte-identical on all five systems.** | Mapping is not reading: a column helps only when a physically named rule consumes it | The remaining 17 misses are coil fouling whose recorded values sit inside every healthy band; the series' claim is that those are limits of what is recorded, not of the detector. **Rule: every mapped column must have a reader or a written reason it has none; and a witness that appears only when the plant saturates cannot be certified from one healthy year.** |
