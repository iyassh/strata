...
| L55 | X31 first pass: every DDAHU condition, including a schedule shift that touches only the occupancy signal, detected 50/55 against the scorecard's 45, the same four scenarios gained each time. Cause: the library facade (`pipeline.fit`) still pooled the residual null over channel-days (X25 step 3, 3/509) while `scripts/benchmark.py` had moved to the per-day null (step 4, 3/124); the regression gate regenerates through the scripts and could not see the drift (the facade test of the time compared one system's flag counts, not the null pair). Facade repaired (X31 Amendment 1), guard `tests/test_facade_residual_null.py`, all arms re-run against a clean arm of the same facade. | The published scorecards were right; the importable library ran a looser gate than they report | **Rule: every experiment that scores through the library needs a clean arm through the same code path, and a guard pins each library statistic to the scorecard that reports it.** |
| L56 | X31 (re-run through the repaired facade against a clean arm): sensor error + 5 % feedback quantisation, change-of-value logging with gaps, schedule shifts, and all three, injected into SDAHU/DDAHU/FCU and every fault file. Budget holds in every cell (worst 3/96, never more than clean + 2); COV logging is the harshest condition (−1, −3, 0 detections), noise costs one on DDAHU and FCU, combined at most one; the schedule shift gains three DDAHU sensor-bias detections at 0 false alarms — P3 failed upward: the noise floor is unchanged but the hot-deck residual band's top falls from 9.28 to 4.87 °F because the one training day that set it became a seeded holiday, and the biased files then flag 87/26/79 residual days instead of 16/7/8. Absence channel not exercised (device stratum off). | The deployed gate survives the field conditions the jitter test did not cover, at a detection cost of at most three per system; the one wrong prediction was in the detector's favour and is not credited to it | X9/X10's robustness claim now extends to bias-like noise, stale logging and moved schedules on three equipment classes. **Rule: a gain under perturbation is a prediction failure to explain, not a result to keep; and an extreme [min, max] band is set by one day, so losing one day can move a detection — a quantile band is the experiment this leaves open.** |
| L57 | X33a (coverage series, SDAHU): audit of all 30 columns — 12 → 26 mapped (fan power, airflow, fan speed and commands, five zone temperatures), 4 excluded with written reasons (two constants, the two E3 leak columns); fan-side specific-power and fan-law residuals, a return-fan mismatch rule, zone-temperature oscillation. 14/14 unchanged, deployed 1/96 unchanged, every new residual 0/79 holdout false alarms, model rows byte-identical; the residual channel now corroborates two stuck-coil scenarios that had rules only. | Full coverage on the reference system costs nothing and adds independent witnesses; the audit-and-remap loop is safe | Configurations must be written from the column list, not the control sequence. **Rule: every recorded numeric column is mapped or excluded with a reason, and a guard enforces it (`tests/test_sensor_coverage.py`).** |
| L58 | X33b (coverage series, PFPU): all 109 columns mapped (53 added), 18 residual rules and a static-setpoint rule from the healthy year. 22 → **26 of 30**: both airside reheat-fouling misses (moderate, severe) through zone discharge-minus-primary airflow with the zone fan off (172 and 227 days), and both room-temperature biases through return-air-minus-zone residuals (187, 261 days); no loss; deployed 7 → 8/96; every new rule ≤ 1 holdout day; model rows identical. P4 (fouling stays missed) failed upward. | Airside fouling is a flow-resistance fault: it moves an airflow relationship the configuration never read, while every temperature the controller holds stays in band | Two of the six 'unreachable' fouling misses were reachable through a column pair nobody had mapped. **Rule: before calling a miss a recording limit, every recorded column must have been read by a physically named residual.** |
| L59 | X33c (coverage series, SFPU): all 109 columns mapped, zone-fan specific-power and pressure-per-flow² residuals per zone. 21 → **25 of 29**: the VAV fan restriction and both airside reheat-fouling misses (severe, moderate) through zone S fan power per cfm (261, 261, 145 days), the +2 °C bias through discharge-minus-supply and return-minus-zone residuals; no loss; deployed 6 → 9/96 (three residual days on four different rules, all kept under the adoption rule); residual-null denominator 41 → 69 (floor 7.3 → 4.3 %), no verdict flipped; model rows identical. | A continuously running zone fan is a permanent witness of downstream airflow resistance: 'same power, less air' is the signature of both a restricted fan and a fouled coil face | Every X32 prediction for this unit held. The one day of budget headroom left is the series' first real cost. **Rule: when a channel's holdout days approach the budget, name the rule that could be removed and what it would cost, in the write-up, before anyone tunes.** |
| L60 | X33d (coverage series, DDAHU): all 114 columns mapped (35 added); mixing-box statics and entering-air temperatures as redundant copies of the deck sensors, deck-fan and pump readers. 45 → **50 of 55**: all three sensor-bias misses (hot-deck static −2/−4 in.wg, hot-deck temperature +2 °C) at 282/282 days each through the box residuals — the deck column stays at setpoint and the four boxes record the true shift: +0.2/+0.4 in.wg (one tenth of the label, ERRATA E8) and −3.6 °F (the label exactly) — heating waterside fouling minor through the hot-water pump bypass (282 days), cooling airside fouling moderate through the pooled union of six fan-side rules (32 days, none significant alone, first alarm day 14); no loss; deployed 3 → 3/96; all 25 new rules 0/75; model rows identical; five scenarios detected earlier. | The dataset records the true quantity at every mixing box while the deck column carries the controller's held reading; the configuration had not read the boxes | Sensor bias is the easiest fault class once redundancy is mapped, and mapping it also measures what was actually seeded. **Rule: map every redundant measurement pair before writing any single-sensor band; then check the seeded magnitude against the label.** |
| L61 | X33e (coverage series, FCU): no unmapped column, but two mapped columns no rule read; readers for outdoor-air flow at the minimum damper position and fan specific power. 40 → **43 of 47**: both remaining damper leaks (20 %, 50 %) at 261/261 days through the outdoor-air flow fraction, and heating waterside fouling moderate through the fan being driven to a speed the healthy year never used (an operating-point witness); no loss; 4 → 5/96; model rows identical. **Series total: 142 → 158 of 175 scenarios, 21 → 26 false-alarm days of 480, nothing lost, model and device rows identical on all five systems.** | Mapping is not reading: a column helps only when a physically named rule consumes it | The remaining 17 misses are coil fouling whose recorded values sit inside every healthy band; the series' claim is that those are limits of what is recorded, not of the detector. **Rule: every mapped column must have a reader or a written reason it has none; and a witness that appears only when the plant saturates cannot be certified from one healthy year.** |
| L62 | Series analysis after X33: all fifteen residual-carried recoveries clear the pre-series noise floor too (the floor's move did not buy them); the sixteenth (FCU heating waterside moderate) clears only the rules channel's 3/365 healthy-year floor; five of sixteen recoveries sit < 1 band width outside the healthy [min, max] extremes; the readers were selected after a read-only scan of the fault files (thresholds blind, selection fault-informed). | The coverage claim is 'reachable from the recorded columns with blind thresholds', not 'a blind onboarding would have reached them'; X17/X19 remain the prior for a new building | **Rule: after any series that adds channels, re-test every gain against the pre-series null, measure its margin in band widths, and state whether the design saw the faults.** |
| L63 | X34 component attribution (pre-registered): the rule carrying each of the 158 detections points at the seeded component exactly in 109 (69 %), the same subsystem in 22 (14 %), the wrong subsystem in 20 (12.7 %), unnamed in 7. P1 held, P2 (≥ 85 %) and P3 (≤ 10 % wrong) failed → **F-X34.a fired: the component-level claim is withdrawn**; the paper reports subsystem-level attribution. Fourteen of the twenty wrong are on the single-zone fan coil unit and name the component that responds to the fault (coil for a biased room sensor, fan for a fouled coil, valve for a reversed controller); five are dual-duct zone-damper faults pointed at the right zone's box but scored against the deck end of the redundant pair. | Where in the HVAC system: subsystem yes, exact component mostly, cause vs response not separable on a single-zone unit | **Rule: an attribution claim needs its own pre-registered bar; 'the alarm names the physics' is not 'the alarm names the cause'.** |
