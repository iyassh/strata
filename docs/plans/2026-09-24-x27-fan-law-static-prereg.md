...
## Note (2026-09-24, before scoring): P0

With `op: ratio_sq` implemented and no config touched, the SDAHU scorecard
regenerated byte-identically; the SDAHU false-alarm artefact differed in
one caveat *string* only (its committed copy predated the X25 caveat
wording, having been written minutes before that edit). Every number is
identical. The refreshed artefact is committed with this note.
