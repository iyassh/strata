...
\title{STRATA: Nine Adverse Results and a Detector\\ \large HVAC fault detection under a falsification protocol}
\author{Yassh Singh \\
...
Every experiment is pre-registered: the hypothesis, and the specific observation that would refute it, are committed to version control before the experiment runs, in the spirit of the hypothesis-testing discipline recently urged for process-mining research \cite{pitsch2025hypothesistesting}. A falsifier that fires is treated as a finding and published, not as a setback to be absorbed by rewriting the hypothesis. Nine results went against the project --- four refuted hypotheses, one fault-informed rule whose falsifier fired, three withdrawn claims, and one pre-registered test that never had attainable power --- several of them results the project would have preferred to keep. Section~\ref{sec:falsified} reports each with the claim it killed and what survived.
...
Four pre-registered hypotheses were refuted during this work, one fault-informed rule fired its pre-registered falsifier, three of the project's own results were withdrawn, and one pre-registered test was found to have no power (Table~\ref{tab:refuted}). They are reported because a protocol that only confirms is not a protocol, and because these refutations constrain the contribution more usefully than the confirmations do.
...
8 & Hypothesis (fault-informed, disclosed): a coil-effectiveness residual, water-side $\Delta T$ per gallon, separates reheat-coil fouling at day level. & Significant on one of twelve fouling scenarios, already detected by other channels; nothing newly detected; falsifier F-X13.b fired. The statistic's day-median distribution overlaps healthy almost entirely (bands 0.6--94 $^\circ$F/GPM) because the ratio explodes at low flow. & The fouling misses need a coil heat-transfer model or a different sensor, not one more residual; eight of the ten missed fouling scenarios carry no signal in the event log at the deployed alphabet, and one of those eight becomes detectable on an enriched one (Section~\ref{sec:timeperspective}). \\
...
\hline
\end{tabular}
\caption{Nine results that went against the project. Rows 4 and 5 are reported in Section~\ref{sec:protocol}; the rest in the text that follows.}
\label{tab:refuted}
\end{table}
...
All results come from simulation, and simulation flatters. Noise-free years make zero-false-positive thresholds cheap, bands razor-thin and silence easy; a substantial share of the project's own recorded lessons reduce to that single observation. Two pre-registered robustness tests bound the flattery without removing it: under i.i.d. per-sample jitter at building-sensor precision (0.5\,$^\circ$F, 2\% of flow, 0.01 of travel; not bias or drift) the re-fitted detector's detection counts are unchanged on all three systems and false alarms stay at 1--4 days in 96, and under the mirrored eight-day holdout the counts move by at most one --- while the membership of the marginal scenarios shifts (at 1$\times$ jitter, two lost and two gained on the parallel unit; one and one under the mirrored holdout), which is the honest reading of a count that looks stable.\footnote{\texttt{outputs/x9\_x10\_robustness.json}; three amendments, each recorded before the result it governs; the fan-powered arms exclude the conformance channels, whose removal changes no clean count.} The deployment phase on real building data remains outstanding, and until it is complete the framework should be considered validated in the laboratory rather than in the field. The contamination result of Section~\ref{sec:falsified} bears directly on that gap, because a real building's baseline year is not fault-free by construction.
...
The evaluation protocol is offered as a contribution in its own right. Four pre-registered hypotheses were refuted, three results withdrawn, one pre-registered test found to have no power, and one fault-informed rule whose falsifier fired, and all nine are reported; the numbered lessons are recorded with the standing rule each produced; five defects in a widely used public benchmark were documented with reproducible evidence; and an adversarial audit voided a completed comparison against commercial practice rather than allowing it to be published. In a subfield where independent verification is rare, a result that has survived a genuine attempt at refutation is worth more than a larger number that has not.
