...
\begin{abstract}
\noindent
A preprint audit of sixty-five machine-learning studies of HVAC fault detection found two that shared code links, one of them broken. We report STRATA, a fault-detection framework for building HVAC systems, together with the falsification protocol under which it was built: hypotheses pre-registered with the observation that would refute them, results recomputed independently from raw data before they are quoted, and automated tests that pin published numbers, fired falsifiers, and disclosure obligations. The protocol refuted the project's founding hypothesis. Process models discovered from fault-free operation were expected to detect faults; a one-line hand-written rule out-covered them, and an ablation over all 73 seeded scenarios finds no fault detected only by the discovered models --- removing them changes no scorecard and lowers one system's false-alarm rate. A sealed, pre-registered test of the fallback claim, that the models predict where a rule transfers between buildings, was left without statistical power by its own pre-computation amendment; it is reported as a protocol failure, not a result. What survives is a detector of eight calibrated, physically interpretable channels that on three public LBNL benchmarks --- two never previously benchmarked --- detects 13 of 14, 23 of 30 and 24 of 29 seeded faults at a median of one day, with a stated false-alarm budget of 1.0--5.2\% of fault-free days (15.6\% on the air handler with every channel counted), naming the correct equipment in 36 of the 46 detections where equipment can be named and a wrong one in none. A principal-component baseline ties it on detection and matches or betters its false-alarm rate; it cannot name the device. The evaluation also produced five machine-verified, previously unreported errata in the benchmark datasets and a 3.5-percentage-point measured bound on model self-confirmation. Every number regenerates from the public repository.
\end{abstract}
...
Every experiment is pre-registered: the hypothesis, and the specific observation that would refute it, are committed to version control before the experiment runs, in the spirit of the hypothesis-testing discipline recently urged for process-mining research \cite{pitsch2025hypothesistesting}. A falsifier that fires is treated as a finding and published, not as a setback to be absorbed by rewriting the hypothesis. Seven results went against the project --- three refuted hypotheses, three withdrawn claims, and one pre-registered test that its own amendment left without power --- several of them results the project would have preferred to keep. Section~\ref{sec:falsified} reports each with the claim it killed and what survived.
...
Implicated device named correctly & 36 of 46 (10 unnamed, 0 wrong; the single-duct system has no device stratum) & not applicable \\
...
\hline
1 & Hypothesis: discovered models out-detect hand-written rules. & A one-line matched rule --- occupied workday and AHU heating never active --- covered 192 days against the model channel's 135 on the same scenario, with zero false alarms on 365 healthy days. The falsifier fired. & The models are design-time scaffolding: they surfaced the invariant, the expert wrote the rule, and thereafter the models contributed no detection. The matched rule is a permanent ablation arm. \\
...
\textbf{A pre-registered test that its own amendment left without power.} After the ablation one claim remained: that the models locate invariants, so that a rule written from them would hold where the model's support for the invariant is high. A sealed test was pre-registered --- the model-derived quantity, predictions and falsifiers fixed and committed before computation; the quantity computed by an agent forbidden from opening the rule-firing artefacts; the scorer committed before data. Before any value was computed, a dated amendment excluded the single-duct system for a correct reason: it has no heating rule and no hot-water valve, so a rule reading `heating never active' fires on every occupied day for want of a sensor, and its 231 firings are not evidence about any model (this also retracts the project's earlier reading of that number). The amendment left six cells in two groups, for which the minimum attainable exact permutation $p$ is $3!\,3!/6! = 0.05$ against a pre-registered criterion of $p < 0.05$. The test therefore could not pass under any data. The amendment recorded the bound and the test was run anyway; the reported $p = 0.20$ is the floor the observed firings permit, not a measurement, and the fired falsifier licenses no conclusion. A second amendment, made after the fact, records this and a post-hoc scorer change. The paper draws no inference about the claim in either direction.

What the exercise did show is reported as observation. The series unit's net treats heating as obligatory --- all 44 fitting healthy variants contain it and none survives its deletion --- while the parallel unit's treats it as optional; the alignment-based support equalled the raw log count exactly on every system, so at the unit stratum the model's reading added nothing over counting; and at the device stratum the pooled net ordered the two buildings the opposite way. Four of the six scored cells were rules whose bands were fitted on the same fault-free year they were scored on, so their near-zero firings are arithmetic rather than transfer; only the parameter-free rule was genuinely transplanted. The one data-dependent outcome, F2 --- the parallel unit's net calls heating optional yet the count-band rule never fired there --- contradicts the structural form of the claim. A powered test needs more buildings, more than two support levels, or an outcome that is not self-fit; it is future work, and the pre-registration that was destroyed by its own legitimate amendment is left in the record as the lesson it is.
