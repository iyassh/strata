...
8 & Hypothesis (fault-informed, disclosed): a coil-effectiveness residual, water-side $\Delta T$ per gallon, separates reheat-coil fouling at day level. & Significant on one of nine fouling scenarios, already detected by other channels; nothing newly detected; falsifier F-X13.b fired. The statistic's day-median distribution overlaps healthy almost entirely (bands 0.6--94 $^\circ$F/GPM) because the ratio explodes at low flow. & The fouling misses need a coil heat-transfer model or a different sensor, not one more residual; the nine scenarios carry no signal in the event log at any perspective (Section~\ref{sec:timeperspective}). \\
\hline
\end{tabular}
