...
The noise-floor gate applies no correction across channels and scenarios. On the order of 584 binomial tests are run at $\alpha = 10^{-3}$ --- eight channels by 73 scenarios, with the device channel additionally run per terminal unit on the fan-powered systems and absent on the single-duct system --- for a family-wise error near 0.44 under independence and an expectation of about 0.6 false cells across the whole study, and the per-day observations within a scenario are autocorrelated, a caveat every artefact carries. Bonferroni correction is applied only across devices within the device channel. Confidence intervals are now given for the false-alarm rates; none is given for the detection counts, which are single-run integers on one simulated year per system.
