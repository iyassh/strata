...
8 & Hypothesis (fault-informed, disclosed): a coil-effectiveness residual, water-side $\Delta T$ per gallon, separates reheat-coil fouling at day level. & Significant on one of twelve fouling scenarios, already detected by other channels; nothing newly detected; falsifier F-X13.b fired. The statistic's day-median distribution overlaps healthy almost entirely (bands 0.6--94 $^\circ$F/GPM) because the ratio explodes at low flow. & The fouling misses need a coil heat-transfer model or a different sensor, not one more residual; eight of the ten fouling scenarios carry no signal in the event log at any perspective (Section~\ref{sec:timeperspective}). \\
...
All results come from simulation, and simulation flatters. Noise-free years make zero-false-positive thresholds cheap, bands razor-thin and silence easy; a substantial share of the project's own recorded lessons reduce to that single observation. Two pre-registered robustness tests bound the flattery without removing it: under i.i.d. per-sample jitter at building-sensor precision (0.5\,$^\circ$F, 2\% of flow, 0.01 of travel; not bias or drift) the re-fitted detector's detection counts are unchanged on all three systems and false alarms stay at 1--4 days in 96, and under the mirrored eight-day holdout the counts move by at most one --- while the membership of the marginal scenarios shifts (two lost, two gained on the parallel unit), which is the honest reading of a count that looks stable.\footnote{\texttt{outputs/x9\_x10\_robustness.json}; three amendments, each recorded before the result it governs; the fan-powered arms exclude the conformance channels, whose removal changes no clean count.} The deployment phase on real building data remains outstanding, and until it is complete the framework should be considered validated in the laboratory rather than in the field. The contamination result of Section~\ref{sec:falsified} bears directly on that gap, because a real building's baseline year is not fault-free by construction.
