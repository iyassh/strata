...
The null of Section~\ref{sec:results} was argued from the model side --- order-permissive nets, count deviations surfacing as alignment cost --- until the log itself was examined. For the parallel fan-powered unit we compared, day by day, the fault-free year against a missed scenario and a detected one on the state alphabet alone. Under waterside reheat-coil fouling at its most severe the state-event log is identical to healthy to the minute: 162.8 against 162.4 events per day, the same activity counts, a zone-S heating-on median of 100 minutes in both. Over all 73 scenarios, at a 5\% tolerance on every activity's daily count and every state's on-duration median, eight of the twelve scenarios the deployed detector misses have a state log indistinguishable from healthy --- all reheat-coil fouling --- and for them no method that reads the event log can detect, process mining or otherwise, because the fault lives in heat-transfer magnitudes the abstraction discards; the other four change a daily count by 7--39\% that the frequency channel's bands did not resolve into significant days.\footnote{\texttt{outputs/x12\_log\_diagnosis.json}.} Under a 50\% reheat-valve leak the log does change --- the zone-S heating-on median falls from 100 to 11 minutes and the zone fan cycles 26.6 times a day against 16.3 --- in timing and in counts, with the activity set and its order unchanged. Control-flow conformance is blind to timing by construction; the frequency channel already reads counts.

That diagnosis fixed a pre-registered test of the one perspective never tried (X12): a time-infused state model --- for every on/off pair of the state alphabet, the day's median on-duration, off-gap and first-on minute, train min/max bands widened by five minutes, the holdout false-alarm floor and the same binomial gate as every channel. Against the committed scorecards' flag days it is significant on 11 of 73 scenarios with $p$ down to $10^{-94}$, on none of the nine fouling scenarios (predicted), and on \emph{no scenario the deployed channels miss} (the load-bearing prediction, refuted); it is never earlier than the deployed detector (typically day 14 against day 1) and it costs five holdout false-alarm days on each fan-powered system, taking the parallel unit's joint rate from 5.2\% to 10.4\%, over budget. What it adds is day-level coverage on scenarios already caught: 120 unique days on the series unit's unstable room sensor (deployed union 19), 82 on the single-duct coil-bias run, through physically legible statistics such as zone-heating on/off medians and the cooling first-on minute.\footnote{\texttt{outputs/x12\_time\_perspective.json}; pre-registration and fourth prediction's weakness disclosed in \texttt{PHASE9\_RESULTS.md}.} A fault-informed coil-effectiveness residual for the fouling misses (X13, design disclosed as chosen after inspecting three fouling files, thresholds from healthy train days only) then fired its falsifier: one of nine fouling scenarios significant, nothing newly detected. The reading is now measured rather than argued: process mining's contribution on these systems is the event log and its count and time perspectives; the discovered net's order structure contributes nothing, because the faults in this benchmark do not change the order of operating states.
