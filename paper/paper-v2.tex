...
%
% PENDING (Sec. "Work in Progress" is a placeholder, no numbers):
...
\begin{abstract}
\noindent
Heating, ventilation, and air conditioning (HVAC) systems account for a substantial share of building energy use, and 15--30\% of that energy is wasted by faults that operate quietly below the alarm threshold of a building's own control system. Existing detection tools face a trust problem: rule libraries flood operators with unexplained alarms, and machine-learning detectors flag anomalies they cannot name. We present STRATA, a framework in which process models discovered from fault-free operation \emph{locate} a building's behavioural invariants --- which device, which unit, which operating regime --- while calibrated, physically interpretable channels \emph{detect} violations of them, so that every alarm carries a name, an explanation, and a measured false-positive budget. This division of labour is not the one we set out to demonstrate: our original hypothesis, that discovered models would themselves detect faults better than hand-written rules, was refuted by its own pre-registered test and is reported here as such. On three public LBNL benchmark systems --- a single-duct air-handling unit and two fan-powered-unit configurations that had never previously been benchmarked --- STRATA detects 13 of 14, 23 of 30, and 24 of 29 seeded fault scenarios at a median time to detection of one day, with a stated joint false-alarm budget of 1.0--5.2\% of fault-free days (15.6\% on the single-duct system if every channel is counted) and a cry-wolf ratio below 0.1\%. A principal-component baseline given the raw sensors ties on detection count and matches or betters that false-alarm rate; what it cannot do is name the implicated device. Removing the process-mining channels changes no detection on any system. A matched-rule control establishes that the models' contribution is discovery automation: hand-written equivalents match individual channels only after the models reveal where each invariant holds, and transplanted blindly they false-alarm on as many as 231 days per year. The evaluation protocol is offered as a contribution in its own right: pre-registered falsifiers whose every firing is reported, a measured 3.5-percentage-point bound on model self-confirmation, five machine-verified errata in the most widely used public FDD datasets, and full regeneration of every published number from a public repository.
\end{abstract}
...
Four contributions follow. The framework is, to our knowledge, the first application of event-log process mining to building HVAC fault detection, a novelty claim corroborated by the one systematic review of process mining on sensor data whose scope could have captured such work \cite{brzychczy2025pmsensorreview} and by its absence from three adjacent surveys. It is evaluated on three public LBNL benchmark systems, two of which had not previously been benchmarked in the literature, at a stated false-alarm budget rather than at accuracy alone. The evaluation protocol --- pre-registration, honoured falsifiers, adversarial recomputation, and full artefact regeneration --- is a methodological contribution in a subfield where an audit of sixty-five primary studies found only two sharing executable code \cite{mukhtar2025reproducibility}. Finally, auditing our own evaluation produced five machine-verified errata in the benchmark datasets themselves. A diagnostic report for facility operators remains the intended deployment artefact, drawing on a planned transfer to real-world data from Thompson Rivers University's Campus Activity Centre, where the framework is designed to complement the rule-based fault detection already present in the building's automation system.
...
STRATA consumes one fault-free year of ordinary building-management-system trend data together with a single configuration file naming the building's sensors and control rules. That configuration file is the only site-specific artefact: no code change was required to onboard any system reported here.
...
Every experiment is pre-registered: the hypothesis, and the specific observation that would refute it, are committed to version control before the experiment runs, in the spirit of the hypothesis-testing discipline recently urged for process-mining research \cite{pitsch2025hypothesistesting}. A falsifier that fires is treated as a finding and published, not as a setback to be absorbed by rewriting the hypothesis. Six results went against the project --- three refuted hypotheses and three withdrawn claims --- including against results the project would have preferred to keep, and Section~\ref{sec:falsified} reports each with the claim it killed and the reframing it forced.
...
The median time to detection is one day on all three systems, but the median alone misrepresents the distribution and should not be quoted without its tail. On the single-duct system nine scenarios are detected within one day and four within two. On the parallel unit nineteen are detected within a day, the remainder at four, eleven, fifteen and twenty-one days. On the series unit nineteen are detected within a day, with the remainder at three, fifteen, fifteen, thirty-seven and one hundred and eight days. Gradually developing faults are detected slowly, which is the expected cost of calibrating below a noise floor and is a genuine limitation for the degradation faults that motivate much of this literature.
...
Against a principal-component-analysis baseline using squared prediction error, audited and scored on comparable scenario universes, the two methods reach a near-tie: 60 scenarios against 61 in total, at 13 versus 13, 23 versus 23, and 24 versus 25 per system. The misses are complementary rather than nested. STRATA alone detects an unstable VAV damper and severe waterside fouling; the baseline alone detects two room-temperature bias scenarios and moderate airside fouling, the latter through a zone-fan differential-pressure sensor that STRATA's configuration does not map. Ten scenarios are missed by both, and nine of them are one family: reheat-coil fouling, airside and waterside, at every severity on the parallel unit and at three severities on the series unit, is invisible to both methods. That is the most useful negative result in the comparison. On false alarms the baseline is at least as good. Its strict holdout rate is 1/79, 1/96 and 1/96 (1.3\%, 1.0\%, 1.0\%; Wilson 95\% intervals [0.2, 6.8], [0.2, 5.7], [0.2, 5.7]) against STRATA's 1/96, 5/96 and 4/96 (1.0\%, 5.2\%, 4.2\%; [0.2, 5.7], [2.2, 11.6], [1.6, 10.2]); the intervals overlap throughout, and an exact McNemar test on detection gives $p = 1.0$. Nor are the inputs symmetric: the baseline consumes 108 and 424 raw daily features on the single-duct and fan-powered systems where STRATA's configurations map 12 of 30 and 56 of 109 columns. What the baseline does not do is name the implicated device or state the evidence behind a flag. That is the whole of the difference, and this paper asserts it rather than demonstrates it: no worked alarm is shown, and Section~\ref{sec:limitations} records that as an open gap.

The matched-rule control isolates what discovery contributes. For each channel a hand-written equivalent was authored and scored. Two of these reproduce their channel exactly --- 141 detections against 141 on the series unit and 206 against 206 on the parallel unit --- which is the point rather than a disappointment: the models automate the discovery of rules an expert could have written, had they known where to look. Transplanted without that knowledge the same rules are unusable, one of them firing on 231 fault-free days on the single-duct system and 124 on the parallel unit while firing on none at all on the series unit --- the contrast that makes the point, since the same rule is harmless exactly where the model had shown the invariant to hold. Some faults, conversely, need the classical instrument: a calibrated oscillation channel stands alone on the unstable-damper scenario, 349 detections against at most two from any matched rule.
...
Three pre-registered hypotheses were refuted during this work and three of the project's own results were withdrawn. They are reported because a protocol that only confirms is not a protocol, and because these refutations constrain the contribution more usefully than the confirmations do.
...
Auditing the evaluation produced an unanticipated secondary contribution: five defects in the LBNL benchmark datasets, each with machine-verifiable evidence and a reproducing command. Two concern single simulation runs shipped four times under different severity labels. One concerns a fault whose labelled bias never appears in the logged column; the evidence is observationally indistinguishable from a bias applied at the controller input rather than at the sensor, which is how the erratum records it. One concerns a file whose calendar is rotated relative to its siblings, excluded here from family claims. The fifth is the configuration-branch mismatch described above, which hid for six phases because every comparison ran against the same fault-free file and no fault-versus-fault control existed.
...
\section{Work in Progress: Comparison Against Guideline 36 Practice}
\label{sec:baseline}
...
A worked example of an alarm --- the name, the evidence, and the budget the abstract promises --- is not shown. The claim that STRATA's output is actionable where a scalar anomaly score is not rests on assertion until one is.
...
Process mining had not previously been applied to fault detection in building HVAC systems, an absence confirmed independently by four systematic reviews. This work establishes that the approach is viable and, more usefully, establishes what it does and does not contribute. Discovered process models did not outperform hand-written rules at detection, and the hypothesis that they would was withdrawn when its own pre-registered test refuted it. What the models do is locate the invariants a particular building holds constant, which makes it possible to calibrate interpretable channels against a measured noise floor and to issue every alarm with a name, an explanation and a stated false-alarm budget. On three public benchmark systems the framework detects the majority of seeded scenarios at a median of one day while false-alarming on 1.0 to 5.2\% of fault-free days, matching a machine-learning baseline on detection count with complementary misses while remaining accountable for its output in a way that baseline is not.

The evaluation protocol is offered as a contribution in its own right. Three pre-registered hypotheses were refuted and three results withdrawn, and all six are reported; thirty-one lessons are recorded with the standing rule each produced; five defects in a widely used public benchmark were documented with reproducible evidence; and an adversarial audit voided a completed comparison against commercial practice rather than allowing it to be published. In a subfield where independent verification is rare, a result that has survived a genuine attempt at refutation is worth more than a larger number that has not.
