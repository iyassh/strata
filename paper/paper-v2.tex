...
These gates were not designed in advance; each was added after a dataset defect got past the previous set. Three of the five errata in Section~\ref{sec:errata} were found by this battery rather than by inspection --- E1 and E2 by the integrity hash, E4 by a dedicated calendar-rotation gate run on the raw files, since the monotonicity check runs on converted files that conversion has already sorted; E3 was found by auditing our own baseline and E5 by a later hostile audit, which is the argument for running it: the defects are not visible in summary statistics, and every one of them would have inflated a published result.
